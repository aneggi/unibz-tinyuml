...
ChangeListener, FocusListener, TreeSelectionListener, UmlModelListener {
  ...
  private int classCounter = 0; // aneggi add class counter inside the app state
  private JLabel classCounterLabel = new JLabel(" Classes: " + this.classCounter);
  ...
  private void installStatusbar() {
    ...
    //statusbar.add(new JLabel("Hello world!"), BorderLayout.CENTER); //aneggi
    statusbar.add(classCounterLabel, BorderLayout.CENTER); //aneggi
  ...
  public void elementAdded(DiagramEditor editor) {
    updateMenuAndToolbars();
    this.classCounter++;
    Component tempComponent = getCurrentFocusedComponent();
    UmlModel tempUMLModel = getUmlModel();
    tempUMLModel.getDiagrams();
    
    classCounterLabel.setText(String.format(" Class Count:" +  this.classCounter));//aneggi
  }
